...
function default_1(file, opts) {
    ...
    if (opts.fd === undefined) {
        fs.open(file, opts.flags, opts.mode, function (err, fd) {
            if (err)
                return fa.emit('error', err);
            fa.fd = fd;
            fa.emit('open', fd);
            for (var i = 0; i < fa._onopen.length; i++)
                fa._onopen[i]();
            fa._onopen.splice(0);
        });
    }
...
FA.prototype._read = onopen(function (start, end, cb, rev) {
    ...
    if (start === undefined)
        start = 0;
    if (start < 0)
        return self._readReverse(start, end, cb, rev);
    ...
    (function _read() {
        fs.read(self.fd, buffer, 0, buffer.length, offset, function (err, bytesRead, buf) {
            if (err)
                return cb(err);
            if (bytesRead === 0) {
                if (rev) {
